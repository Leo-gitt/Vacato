...
const COLOR_STYLES = {
  ...
  stone: { icon: 'bg-stone-500/15 text-stone-400', border: 'border-l-stone-500', bar: 'bg-stone-500' },
...
export function StatCard({ label, value, sub, color = 'stone', icon: Icon, used, total }: StatCardProps) {
  ...
  return (
    <Card className={cn('border-l-4 border-stone-800 bg-[#333333] p-5 shadow-lg shadow-black/20', style.border)}>
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <p className="text-[11px] font-semibold tracking-widest text-stone-400 uppercase">{label}</p>
